...
def is_direct_message(update):
    return update.message.chat.id == extract_user_data_from_update(update).get('user_id')

def is_admin(update):
    return update.message.chat.get_member(
        extract_user_data_from_update(update).get('user_id')
    ).status != 'member'